...
- 拖拽、缩放、状态切换或 surface 变化前，主进程通过布局事务把跑道内位置实体化为普通宠物窗口 bounds；renderer 在 resize 前先隐藏旧 sprite 并等待透明帧完成绘制，原生 bounds 提交后再在匹配的目标 viewport 应用布局、恢复 sprite 并确认 painted，避免 Windows 合成器把旧动作帧暂时放到跑道左端。walk 拖拽释放时继续按最终可见中心恢复跑道，避免把窗口左边界误作行走中心造成横向跳变。
