...
test("dock-controller 不再声明内部 windowRoam* 状态副本", () => {
  const forbiddenLets = [
    ...
    "let windowRoamStartSuppressedUntil",
...
test("dock-controller context 包含必要 getter/setter 访问器与协作方法", () => {
  ...
  // 4 个 window-roam-controller 协作方法（状态由 window-roam-controller 统一维护）
...
test("dock-controller 内部使用协作方法调用而非 windowRoam* 赋值", () => {
  ...
  assert.doesNotMatch(codeWithoutComments, /windowRoamStartSuppressedUntil\s*=/);
  ...
  assert.match(codeWithoutComments, /markManualTaskbarHold\(/);
