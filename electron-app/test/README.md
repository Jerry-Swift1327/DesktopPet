...
| 文件 | 覆盖内容 | 相关源码 |
| --- | --- | --- |
| `walk-controller-window-frame.test.cjs` | 固定透明跑道回归：窗口表面逐步只推进 `spriteOffsetX`、方向镜像受边界约束、速度不依赖 frameStep、从普通窗口 bounds 初始化可见中心，布局事务开始暂停后不再提前推进 sprite | `../electron/behavior/walk-controller.cjs` |
...
| `devtools-variant-workflow.test.cjs` | devtools workflow（新增宠物预览/执行、宠物库检查/图鉴、每动作帧段、维护列表/详情、批量动作替换、新增或删除动作资源、删除测试宠物） | `../devtools/services/variant-workflow.cjs` |
| `devtools-runtime-build-workflow.test.cjs` | DevTools 本地启动/停止与 Windows 打包命令白名单、进程状态、日志和受控产物目录 | `../devtools/services/runtime-build-workflow.cjs` |
...
| `frame-geometry.test.cjs` | 帧纯几何计算（spriteRect、visibleInsets、frameVisibleRect、stableGroundBottom、bottomAnchor、centerWindowX、frameStep 序列索引、`freezeLastFrame` 定格） | `../electron/pet/frame-geometry.cjs` |
...
| `surface-fit-rules.test.cjs` | surface-fit 纯规则（visibleTop/groundedWindowY、clampWindowPositionToSurface、getScaleCandidateForSurface 候选适配、edge/center 互推、taskbar/window walk center limits、windowSurfaceWalkRunwayBounds、safeWindowXForDirection、validateWindowSurfaceBounds、getSurfaceGroundYFromSurface） | `../electron/pet/surface-fit-rules.cjs` |
...
| `drag-controller-accessor.test.cjs` | drag-controller 控制器访问器护栏与 main.cjs 薄包装接线结构断言（不直接 require electron/fs/path、不注册 IPC、不直接访问窗口/IPC/bubble、createDragController 导出、context 注入完整、walk 拖拽等待跑道实体化后启动、`windowDocking` 关闭时拖拽释放留在原位、main.cjs 薄包装与 IPC 契约不变） | `../electron/behavior/drag-controller.cjs`、`../electron/main.cjs`、`../electron/ipc/register-ipc-handlers.cjs`、`../electron/behavior/dock-controller.cjs` |
...
| `surface-scale-anchor.test.cjs` | surface-scale-controller 缩放锚点行为回归（缩放前后可见中心保持不变、window surface 固定跑道在缩放前实体化、可见底边保持贴合 surface、窗口尺寸按 scale 更新） | `../electron/pet/surface-scale-controller.cjs` |
| `auto-start-controller.test.cjs` | auto-start 控制器行为回归（已加载偏好时仍按注册表真实状态同步回偏好，启动前同步持久化注册表状态，注册表读取失败时不覆盖偏好，PowerShell registry read command avoids trailing `$args`） | `../electron/platform/auto-start.cjs` |
| `auto-start-controller-accessor.test.cjs` | auto-start 控制器边界与 main.cjs 薄包装接线结构护栏（13 个断言：controller 不直接 require electron/fs/path、不匹配 ipcMain/petWindow/safeSend/broadcastToWindows/showBubbleMessage、导出 createAutoStartController、不声明 autoStartEnabledCache/autoStartPreferenceLoaded 业务状态、声明 autoStartRefreshInFlight 运行态、context 注入 10 个必要依赖、不导出 buildAutoStartSummary/canToggleAutoStart、导出 7 个函数、main.cjs 引入并构造 autoStartController、main.cjs 不再声明 3 个 auto-start 状态变量、6 个 function 转薄包装委托 autoStartController、buildAutoStartSummary 委托 preferencesStore、readAutoStartPreference/writeAutoStartPreference 委托 preferencesStore） | `../electron/platform/auto-start.cjs`、`../electron/main.cjs` |
