...
| `pet2612-assets.test.cjs` | pet2612 yawn 末帧冻结元数据及 manifest 一致性 | `../../assets/animations` |
...
| `drag-controller-accessor.test.cjs` | drag-controller 控制器访问器护栏与 main.cjs 薄包装接线结构断言（不直接 require electron/fs/path、不注册 IPC、不直接访问窗口/IPC/bubble、createDragController 导出、内部持有 3 个运行态、context 注入完整、导出 9 个函数、`windowDocking` 关闭时拖拽释放留在原位、main.cjs 构造 dragController、main.cjs 不再声明运行态、6 个薄包装委托 dragController、6 个控制器 context 的 getDragState/getLastDragSample 改走 dragController、dockPetAfterDrag/applyDockSurfaceAfterDrag 仍委托 dockController 不被内联、IPC 契约不变） | `../electron/behavior/drag-controller.cjs`、`../electron/main.cjs`、`../electron/ipc/register-ipc-handlers.cjs`、`../electron/behavior/dock-controller.cjs` |
