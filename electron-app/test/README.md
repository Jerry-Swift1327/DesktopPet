...
| `bubble-position.test.cjs` | 启动气泡锚点冻结行为结构断言（可见时冻结锚点、resize 不刷新锚点、repositionStartupBubbleWindow 按 refreshAnchor 参数刷新、setPetWindowPosition 触发 reposition、showBubbleMessage 刷新锚点、hideStartupBubble 清空锚点）；setPetScale 函数体改读控制器源，保留 repositionStartupBubbleWindow({ refreshAnchor: true }) 断言 | `../electron/windows/bubble-controller.cjs`、`../electron/pet/surface-scale-controller.cjs` |
