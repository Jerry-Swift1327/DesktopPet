...
function getDragStartFlowBody() {
  return `${extractFunctionBody(dragSource, "handleDragStart")}\n${extractFunctionBody(dragSource, "beginDrag")}`;
}
...
test("handleDragStart 校验 isScreenPoint(point)", () => {
  const body = getDragStartFlowBody();
