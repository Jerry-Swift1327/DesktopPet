...
| `variant-cli.cjs` | 查询变体、按 species/date/scope 筛选、新建 V3 变体、bootstrap 动作源视频、资源检查、维护预览/应用纯函数和生成本地图鉴 |
...
npm.cmd run variant:new -- --species cat --scope custom --date 2026-07-06
npm.cmd run variant:new -- --species cat --scope test --date 2026-07-06
...
变体元数据使用 V3 字段：`species`、`scope`、`notes`、`assetPrefix`、`actions.enabled` 和 `features.enable/disable`。`notes` 由 `scope` 生成默认值，custom 和 test 默认版本 `1.0`，internal 版本按当前最大 internal 版本递增。`squat`、`walk`、`feed`、`ball` 是所有变体的系统必需动作；正式变体 ID 使用 `pet<yy><seq>`，测试变体使用独立的 `pettest<seq>`。

`variant-cli.cjs` 还导出维护中心复用的纯函数：资源检查结果、动作源视频重命名计划/应用、动作新增/替换计划、元数据编辑 diff 预览/应用、测试变体删除预览/应用。新增动作计划使用 `process` 创建尚不存在的资源目录，已有动作替换计划使用 `replace`；元数据预览可由受信任的维护工作流声明本次计划生成的动作资源，从而先处理视频再写元数据。删除函数只允许 `scope: "test"` 变体，并在应用前校验删除路径位于允许的资源根目录内。
