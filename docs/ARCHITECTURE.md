...
截至第十四轮完成，`main.cjs` 约 3841 行，已删除 12 个无调用方的 helper（如 `getActionAssetFolder`、`getWalkArea`、`groundPetToWorkArea` 等），自启动 5 个函数已转为薄包装委托 `autoStartController`。`setState` 已迁出（第十三轮 state-controller），`applySurfaceScale`/`groundPetToSurface` 已迁出 `pet/surface-scale-controller.cjs`（第十四轮），`main.cjs` 保留薄包装委托。
