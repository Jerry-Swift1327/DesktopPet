...
| `pet2613` | `pet2613_squat`、`pet2613_walk`、`pet2613_feed`、`pet2613_ball`、`pet2613_yawn` |
| `pet2614` | `pet2614_squat`、`pet2614_walk`、`pet2614_feed`、`pet2614_ball`、`pet2614_yawn` |
...
新增定制变体时，推荐通过 `electron-app` 下的 bootstrap CLI 生成 V3 元数据、复制源视频并调用资源处理脚本。默认 dry-run，传入 `--apply` 后才写文件：
...
- `electron-app/electron/pet-action-registry.json` 维护统一动作池，`pet-variant-metadata.json` 维护 V3 变体元数据，`pet-catalog.cjs` 和 `pet-variants.cjs` 展开动作顺序、播放语义和打包资源列表。
