...
当前项目已迁移为独立根目录，后续开发默认以 `C:\Users\86183\Documents\DesktopPet` 作为工作区。旧的 `New project\DesktopPetPackage` 仅作为历史参考，不再作为当前开发入口。
...
依赖目录 `electron-app/node_modules` 是本机生成产物，已加入忽略规则，不提交 Git。
...
迁移后的中文乱码和主进程语法错误已修复；进入 macOS 适配前，应继续保持文档、运行入口和实际目录结构一致。
