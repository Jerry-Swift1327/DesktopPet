...
- 亲密度、饱食度、健康值等本地状态。
- 多宠物变体资源：`dog`、`cat`、`shorthair`、`tabby`、`brit`、`bshmitted`、`van`、`pomeranian`。
...
Windows 打包脚本只能在 Windows 上执行，可通过 `-PetVariant` 指定 `dog`、`cat`、`shorthair`、`tabby`、`brit`、`bshmitted` 或 `van`。`PET_VARIANT` 本地启动调试不受打包平台限制。macOS 打包说明见 `docs/MAC_BUILD.md`。
