...
- 亲密度、饱食度、健康值等本地状态。
- 多宠物变体资源：`dog`、`cat`、`shorthair`、`tabby`、`ragdoll`、`brit`、`bshmitted`、`van`、`pomeranian`。
...
Windows 打包脚本只能在 Windows 上执行，可通过 `-PetVariant` 指定 `dog`、`cat`、`shorthair`、`tabby`、`ragdoll`、`brit`、`bshmitted` 或 `van`。`PET_VARIANT` 本地启动调试不受打包平台限制。macOS 打包说明见 `docs/MAC_BUILD.md`。
...
| 类型 | 位置 |
| --- | --- |
| Windows 便携版 | `electron-app/deliverables/<audience>/<variant>/release` |
| Windows 安装向导 | `electron-app/deliverables/<audience>/<variant>/installer` |
