...
- 亲密度、饱食度、健康值等本地状态。
- 多宠物变体资源：`dog`、`cat`、`shorthair`、`tabby`、`ragdoll`、`brit`、`bshmitted`、`van`、`pomeranian`，新增定制变体使用 `<breed>-<yy><seq>` ID。
...
新增定制变体时先选择精简品种名，再由 CLI 生成顺序 ID。历史编号放在 `aliases` 中，CLI 和打包参数可以输入真实 id 或 alias，内部都会解析为真实 id：
...
定制变体使用 `custom/<breed>/<id>/<channel>` 打包路径；视频进入项目后统一命名为 `<id>_<action>.mp4`。
...
| 类型 | 位置 |
| --- | --- |
| Windows 便携版 | `electron-app/deliverables/<scope>/<breed>/<id>/release` |
| Windows 安装向导 | `electron-app/deliverables/<scope>/<breed>/<id>/installer` |
